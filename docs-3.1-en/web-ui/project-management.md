---
title: Project Management
---


# Project Management

GNS3 Web UI is a browser-based network simulation management interface. After you log in, the first page you see is the project page.

- **Top left corner**: The GNS3 Logo is the main menu button. Click it to open a menu with these items:
  - `Controllers`
  - `Projects`
  - `Template Preferences`
  - `Computes`
  - `Image Manager`
  - `New Template`

![Main menu](/img/web-ui/zh/main-menu.jpeg)

- **Top right corner**: More menu (three dots). Click it to open a menu with these items:
  - `System Status`
  - `AI Profile`
  - `Settings`
  - `Management`
  - `Help`
  - `User Info`
  - `API Documentation`
  - `Logout`

![More menu](/img/web-ui/zh/more-menu.jpeg)

- **Middle area**: The project list. You can do these actions:
  - `Add Blank Project`
  - `Import Project`
  - `Refresh Projects`
  - For each project: `Open` / `Edit` / `Copy` / Export / Delete

![Project actions](/img/web-ui/zh/project-actions.jpeg)
