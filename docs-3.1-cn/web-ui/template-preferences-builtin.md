...
![进入 Built-in](/img/web-ui/zh/builtin-enter.jpeg)

Built-in 模板包含以下三种内置网络设备：

![内置分类列表](/img/web-ui/zh/builtin-categories.jpeg)
...
1. 点击"Ethernet Hub Templates"选项进入。
   ![进入 Ethernet Hub](/img/web-ui/zh/hub-create-01-enter.jpeg)

2. 点击右上角的 + 号按钮添加新模板。
   ![点击添加](/img/web-ui/zh/hub-create-02-add.jpeg)

3. 输入模板名称（Template Name）和端口数量（Number of Ports），点击 "Add Template" 完成创建。
   ![配置模板](/img/web-ui/zh/hub-create-03-configure.jpeg)

4. 创建完成后，在列表中可以看到新模板。
   ![创建完成](/img/web-ui/zh/hub-create-04-created.jpeg)

5. 点击右侧的三个点按钮，可以选择删除（Delete）或编辑（Edit）。
   ![操作菜单](/img/web-ui/zh/hub-create-05-menu.jpeg)
...
6. 点击 Edit 进入编辑界面，可配置以下内容：
   ...
   ![编辑模板](/img/web-ui/zh/hub-create-06-edit.jpeg)
...
1. 点击"Ethernet Switch Templates"选项进入。
   ![进入 Ethernet Switch](/img/web-ui/zh/switch-create-01-enter.jpeg)

2. 点击右上角的 + 号按钮添加新模板。
   ![点击添加](/img/web-ui/zh/switch-create-02-add.jpeg)

3. 输入模板名称（Template Name）和端口数量（Number of Ports），点击 "Add Template" 完成创建。
   ![配置模板](/img/web-ui/zh/switch-create-03-configure.jpeg)

4. 创建完成后，在列表中可以看到新模板。
   ![创建完成](/img/web-ui/zh/switch-create-04-created.jpeg)
...
5. 点击 Edit 进入编辑界面：
   ...
   ![常规设置](/img/web-ui/zh/switch-create-05-general.jpeg)
   ...
   修改后，点击右下角的 "Save" 按钮保存生效。
   ![端口设置](/img/web-ui/zh/switch-create-06-port-settings.jpeg)
...
*Access 端口（接入端口）* - 属于单个 VLAN，通常连接终端设备（PC、服务器）

*dot1q 端口（802.1Q 中继端口）* - 允许多个 VLAN 通过，设置一个本征 VLAN（native VLAN）

*QinQ 端口（802.1ad 运营商中继）* - 双层 VLAN 标记，用于运营商网络。可配置外层以太网类型
...
1. 点击"Cloud Node Templates"选项进入。
   ![进入 Cloud Node](/img/web-ui/zh/cloud-create-01-enter.jpeg)

2. 点击右上角的 + 号按钮添加新模板。
   ![点击添加](/img/web-ui/zh/cloud-create-02-add.jpeg)

3. 输入模板名称（Template Name），点击 "Add Template" 完成创建。
   ![配置模板](/img/web-ui/zh/cloud-create-03-configure.jpeg)

4. 创建完成后，在列表中可以看到新模板。
   ![创建完成](/img/web-ui/zh/cloud-create-04-created.jpeg)
...
5. 点击 Edit 进入编辑界面：
   ...
   ![常规设置](/img/web-ui/zh/cloud-create-05-general.jpeg)

6. Ethernet Interfaces — 在模板中没有数据，需要在项目中添加节点后配置。
   ![Ethernet 接口](/img/web-ui/zh/cloud-create-06-ethernet.jpeg)

7. TAP Interfaces — 添加 TAP 接口名称。
   ![TAP 接口](/img/web-ui/zh/cloud-create-07-tap.jpeg)

8. UDP Tunnels — 创建 UDP 隧道连接远端主机的指定端口。
   ![UDP 隧道](/img/web-ui/zh/cloud-create-08-udp.jpeg)
...
*Ethernet 端口（最常用）* - 直接绑定到宿主机的一个物理或虚拟网卡

*TAP 端口* - 绑定到宿主机 TAP 虚拟接口

*UDP 端口* - 直接指定 UDP 隧道端点（极少用）
