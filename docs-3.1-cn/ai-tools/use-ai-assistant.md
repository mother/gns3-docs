...
<img style={{ width: '100%' }} alt="Controller Management入口" src={useBaseUrl('img/web-ui/zh/ai-config-management.jpeg')} />

如下图所示，点击 `Users` 标签，管理员可以为不同的用户进行 `AI Profile` 配置。

<img style={{ width: '100%' }} alt="Users标签配置" src={useBaseUrl('img/web-ui/zh/ai-config-users-tab.jpeg')} />
...
<img style={{ width: '100%' }} alt="Groups标签配置" src={useBaseUrl('img/web-ui/zh/ai-config-groups-tab.jpeg')} />
...
3. AI 助手会解析您的需求，生成对应的设备配置命令，并在确认后自动下发到设备。
